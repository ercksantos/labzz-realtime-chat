  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      if (err.response?.data?.data?.requires2FA && err.response?.data?.data?.userId) {
        ...
        router.push(
          `/2fa?userId=${err.response.data.data.userId}&returnUrl=${encodeURIComponent(returnUrl)}`
        );
        return;
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-dark-bg py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-2">💬 Labzz Chat</h1>
