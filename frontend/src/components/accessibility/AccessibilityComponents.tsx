...
export function LiveRegion({
  ...
  return (
    <div aria-live={priority} aria-atomic={atomic} aria-relevant={relevant} className="sr-only">
      {children}
    </div>
  );
}
...
export function Heading({ level, children, className, id }: HeadingProps) {
  ...
  const headingClass = cn('text-gray-900 dark:text-white', sizeClasses[level], className);
  ...
  switch (level) {
    ...
    case 5:
      return (
        <h5 id={id} className={headingClass}>
          {children}
        </h5>
      );
    case 6:
      return (
        <h6 id={id} className={headingClass}>
          {children}
        </h6>
      );
  }
